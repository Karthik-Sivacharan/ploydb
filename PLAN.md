...
- ~~Vercel AI SDK / API routes~~ — Korra is a scripted UI demo, no real AI
...
```bash
npm install @faker-js/faker date-fns lucide-react next-themes motion class-variance-authority clsx tailwind-merge
```
...
### Programmatic API (for scripted Korra demo)
...
// Filter/sort via TanStack Table
...
└────────────────┬────────────────┬────────────────┘
    ...
    ┌────────────▼──────┐  ┌─────▼──────────────┐
    │   tablecn          │  │   Korra Panel       │
    │   Data Grid        │  │   (scripted UI)     │
    │                    │  │                     │
    │   TanStack Table   │  │   Hardcoded chat    │
    │   + cell variants  │  │   messages +        │
    │   + all grid ops   │  │   setTimeout →      │
    │                    │  │   setData()         │
    └────────────────────┘  └─────────────────────┘
```
...
## Phase 7: Korra AI Panel (Scripted UI Demo)

No real AI — purely choreographed UI:
...
### 7.2 — Scripted Demos
- [ ] Hardcoded message sequences triggered by keyword:
  - "enrich leads" → `setTimeout` → batch `setData` updates
  - "show hot leads" → applies filter via TanStack Table
  - "sort by deal size" → applies sort
  - "qualify stale leads" → updates status on old leads
- [ ] Each "tool call" animates a result card in chat

### 7.3 — AI Trust Signals
...
│   ├── page.tsx                  # Main page — useState + DataGrid
